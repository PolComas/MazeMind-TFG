...
import { loadSettings, saveSettings, type AppSettings, type VisualSettings, type ScreenSettings } from '../utils/settings';
import { ArrowLeft, Save, ChevronDown, ChevronUp } from 'lucide-react';

import HomeScreenSettings from '../components/settings/HomeScreenSettings'; 
// Importarem els altres quan els creem
...
export default function SettingsScreen({ onBack }: Props) {
  // Estat per a la configuració actual
  const [currentSettings, setCurrentSettings] = useState<AppSettings>(() => loadSettings());
  // Estat per a la secció activa de l'acordió
      ...
      <div style={styles.contentGrid}>
        ...
        <aside style={styles.previewColumn} aria-label="Vista prèvia en temps real">
           <div style={styles.previewHeader}>
            <span>👁️ Vista Prèvia en Temps Real</span>
          </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  accordionColumn: { background: PALETTE.surface, border: `1px solid ${PALETTE.borderColor}`, borderRadius: 16, padding: 'clamp(16px, 3vw, 24px)', boxShadow: PALETTE.shadow, display: 'flex', flexDirection: 'column', gap: '12px' }, // Reduït el gap
    accordionItem: {
  ...
  accordionContent: {
    ...
    borderTop: `1px dashed ${PALETTE.borderColor}`,
    marginTop: '8px',
  },
